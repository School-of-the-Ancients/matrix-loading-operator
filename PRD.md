...
4. On the ordinary `/web/` desktop virtual floor, optionally select an existing built-in chair or table and start Citizens without replacing the user's scene. Add only the two resident orbs, show blocked or unsupported choices, and grant a need benefit only after an observed MatrixWorld outcome.
