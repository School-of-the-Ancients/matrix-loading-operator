...
Validation follow-up to merged PRs #3 and #4, targeting `codex/quest-pro-ai-validation`. The installed runtime/build baseline remains `d130e9221a23f73d3bd455fe608e3f3c6a8f15f3`. The actual Quest Pro passed **21 direct command/save/load checks** and now **61 live Codex AI loop checks**. The wearer confirms seeing the AI changes and their scene restored. MRUK hardware remains pending.
...
| Wearer observations | White room and selection confirmed earlier; after the successful AI run: “Yes, I saw the AI changes and my scene is restored.” Full tracking/focus, floor-height, button/stick and comfort checks are not inferred |
...
| First live Codex attempt | **27 checks passed, 1 failed**. Spawn acknowledged, then resize proposal HTTP 409; original three objects restored. Preserved as historical evidence |
| Successful live Codex run | **61 checks passed, 0 failed**, five real turns, 21:29:28.530–21:30:15.351 UTC. Reviewed spawn, double scale on the same ID, PC save, clear and exact restore; actual Quest package PID 10902 |
| Successful-run preservation | Original one-table scene and selection restored. Backup: `CodexBackup_20260920_212928_4798309c`; test save: `CodexLoop_20260920_212928_4798309c` |
...
Direct device evidence is [headset-loop-results.json](headset-loop-results.json). Attribution combines the verified Quest Pro package PID, no competing desktop player, the service becoming connected after USB forwarding, and the wearer's launch report. No desktop substitute or synthetic runtime snapshots were used. The successful AI run is [codex-headset-loop-success.json](codex-headset-loop-success.json); the [first attempt](codex-headset-loop-results.json) remains separate. Existing unit evidence is [codex-service-tests.txt](codex-service-tests.txt). No source changes, rebuild or unit-test rerun was needed for the successful retry. Wearer observations, the 129-check Unity builds and 95 Windows-player checks remain distinct evidence in [White-Room-Validation.md](White-Room-Validation.md).
...
## Live AI pass: completed loop and earlier attempt
...
The later run at 21:29:28.530–21:30:15.351 UTC passed **61 checks, 0 failed**. All five Codex turns completed with zero tool calls: spawn chair, double its scale while keeping the same ID, save the scene, clear it, and restore exact IDs/assets/anchors/transforms. Each proposal was reviewed and applied through the existing service; runtime changes received actual headset acknowledgements. Finally, the one original table and its selection were restored. The wearer then confirmed seeing the AI changes and their scene restored. This supersedes the first attempt's incomplete-loop status without changing its report.

To use it, refresh the Operator page, choose **Codex (ChatGPT subscription)**, enter a request, select **Create proposal**, review, then **Apply reviewed proposal**. Keep controller/browser edits idle during inference and wait for the acknowledgement before the next request. The browser initially had an older offline-only page cached; refreshing exposed the Codex selector. Detailed tracking/comfort acceptance remains separate. See the [Desktop guide](../Docs/Desktop-Unity-Hub.md) for the exported projects and service location.
