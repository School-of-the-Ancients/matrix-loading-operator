...
`matrix_list_assets` reads a bounded page of the existing GLB catalog. `matrix_register_glb` registers an already exported PC-local `.glb` after Codex provides its SHA-256 digest and the wearer approves the native MCP request. The service stages the exact file bytes, checks the digest, and applies the same GLB validator and content-addressed catalog used by the existing authoring paths. Registration alone does not change the scene. Once the connected browser has refreshed that asset, `matrix_spawn_asset` can queue a virtual-room spawn after a separate native approval. `matrix_spawn_status` reports success only after a browser receipt and observed new object ID. The optional MCP dependency is required for these tools; Blender modeling and export still happen through Codex's separately configured Blender MCP.
