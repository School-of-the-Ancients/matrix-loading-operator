...
Codex may request approval for an MCP call. Agent Portal now carries this native approval to the existing in-world Approve/Deny controls. Only the exact read-only `matrix_scene_summary` call is reviewable and approvable in XR; other MCP calls show a generic PC-review message, with Deny and Stop available. This prevents a Blender or other MCP write from being approved on the basis of an incomplete headset summary. A later slice can add explicit review contracts for individual write tools.
