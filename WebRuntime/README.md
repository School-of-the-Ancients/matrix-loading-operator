...
When a saved room origin cannot be located, or a saved world has objects but its anchor handle is missing, `/web/` hides the old virtual world and pauses editing. The in-world World page offers **Retry Saved Room Origin**, **Archive + Place Here**, and **Archive + Start Empty** as applicable. The latter two need a second confirming tap. Place Here deliberately positions the existing virtual world at a new tracked room origin; Start Empty clears the active world. Both first save the old scene, game, and anchor handle in a local recovery archive. If archiving fails, the reset stops without changing the world. The new room stays read-only until its anchor is tracked. At most three archives are retained; export them with **Download room recovery archives** from the browser panel before clearing site data. These recovery actions still need Quest wearer validation.
...
With Codex AI on PC selected, ask for a game such as “Create a game where I return three orbs to a pedestal.” Codex returns a reviewed declarative specification with pickup and delivery-zone roles, release-near rules, and delivered-count objectives. Apply spawns role instances as ordinary Matrix scene objects and binds their object IDs to the game. Grab a pickup and release it within the rule's distance of a matching delivery zone to score. The Operator shows objective progress and a win state. Multiple role and zone pairs can express matching-object games using the same mechanics.

Supported mechanics are pickup roles, delivery-zone roles, release-near scoring, and delivered-count objectives. The planner must report unsupported requests honestly. Chasing enemies, combat, weapons, physics systems, pathfinding, and arbitrary executable scripts are not implemented. Game planning currently selects assets already in the catalog; create and register a missing asset through Blender before requesting a game that depends on it.
