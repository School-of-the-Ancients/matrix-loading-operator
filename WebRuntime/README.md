...
This runtime slice establishes the execution contract. Publishing immutable package IDs and attaching them from the PC-local Codex MCP will be layered on the existing Agent Portal gateway; source-controlled runtime development remains the fallback for a capability that these numeric primitives cannot express. This schema does not yet provide shaders, physics, events, audio, animation clips, or arbitrary behavior code.
