...
The AR scene starts with a copy of the virtual-room objects. It measures the Quest Space Setup floor and places the virtual floor at that height. The wearer confirmed this corrected the robot and other objects appearing at head height in AR. When available, the client saves a WebXR persistent anchor as a shared origin for virtual-floor objects across AR and VR sessions; the in-world Operator panel reports whether that origin is located. If a saved AR origin cannot be restored, the browser hides the old virtual-floor objects instead of silently moving them to the headset's current pose. VR uses the previously measured eye-to-floor height if an anchor is unavailable, but this fallback cannot establish the same horizontal position or direction in the physical room. Cross-session anchor restoration and VR alignment still need headset confirmation. Objects explicitly placed on measured planes remain tied to this WebXR session. When Quest replaces plane objects within that session, the client keeps an ID only if label and measured shape uniquely match; ambiguous matches pause editing for recovery. Ending AR returns virtual-floor edits to the desktop scene. A saved AR scene cannot be restored into a later browser session because these plane IDs are not persistent spatial anchors. The browser does not provide MRUK geometry, physical-camera screenshots, depth occlusion, or native Quest feature parity. Room planes, aligned outlines, virtual objects, and a spoken Blender request that imported a robot were observed on Quest 3. A virtual-only capture from the live Quest Browser rendered the robot and scene objects. PC-generated voice playback and stable placement after relocalization still need headset validation.
