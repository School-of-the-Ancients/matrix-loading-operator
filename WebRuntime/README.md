...
The GLB validator now records up to eight named animation clips with bounded channel, sample, and duration budgets. The browser checks that the loaded clips match the catalog and uses `SkeletonUtils` to clone each rig independently. If an asset has exactly one clip, virtual-floor instances loop it automatically; an instance attached to a measured physical support stays static because an animated footprint has not been validated against that surface. Multi-clip assets remain static until an explicit clip binding contract exists. The clip restarts when an object is reconstructed after a scene edit or reload; playback phase is not saved. This is GLB clip playback, not a claim that the bounded Blender blueprint path can author rigs or that selection interactions already work.
