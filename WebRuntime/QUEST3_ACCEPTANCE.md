...
The results below remain **unverified unless a mode-specific result cites wearer evidence**. Partial checks do not pass the complete Agent workflow. The [September 25 AR trace](../Validation/WebRuntime-Desktop-2026-09-25.md#live-quest-3-ar-trace) records visible animated assets, earlier ray/grab failures, and a limited automatic-approval check. It does not verify the updated ray/grab behavior, a complete Agent workflow, or VR.

| Run | Device/browser, URL and revision | PC mode and XR permissions | Result/evidence link |
| --- | --- | --- | --- |
| VR | Quest 3 / Quest Browser `152.0.0.44.30.1069357998`, Android 14, `http://127.0.0.1:18772/web/`, #90 runtime bundle `index-B_6S6Mfy.js` | PC approval/access mode and granted XR features unrecorded | [Partial entry, floor and Codex voice checks](../Validation/WebRuntime-M4-XR-Transition-2026-09-25.md#quest-retest) |
...
| --- | --- | --- | --- |
| Entry and legibility | Record `immersive-vr` or `immersive-ar` entry, granted features, panel distance/contrast, readable conversation and activity text, and any obstruction. | Partial: same-page VR → AR → VR entry and real floor height confirmed; panel legibility/features unverified. [Quest retest](../Validation/WebRuntime-M4-XR-Transition-2026-09-25.md#quest-retest) | Unverified |
...
| Voice | Record microphone permission, hold/release input, transcript, and a follow-up in the same thread as a typed browser request. If unavailable or denied, record the exact status and verify typing remains usable. Speech output, if enabled, is distinguishable from input. | Partial: wearer held/released CODEX push-to-talk in VR; labels changed and exactly one turn appeared. Transcript, typed follow-up and microphone permission not yet recorded. [Quest retest](../Validation/WebRuntime-M4-XR-Transition-2026-09-25.md#quest-retest) | Unverified |
