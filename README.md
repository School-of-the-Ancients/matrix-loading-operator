...
**New experimental client:** [Matrix Web Runtime](WebRuntime/README.md) runs the same Operator scene schema through Three.js/WebXR. It keeps Unity as a supported native runtime, adds a content-addressed GLB catalog for browser hot loading, and can build three bounded procedural asset shapes on request. Quest 3 Browser has reported aligned room planes and completed a local voice transcription and Codex response; in-world panel visibility, wall pinning, object visibility after relocalization, and performance still need headset validation.
