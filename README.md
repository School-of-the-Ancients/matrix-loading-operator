...
Matrix Loading Operator builds and edits scenes from seven bundled prefabs using a PC-hosted Codex provider. The working **virtual white room** supports controller selection, push-to-talk, model/reasoning selection, reviewed edits, undo/redo, and PC save/load. Quest Pro is the hardware target; Quest 3 is optional.

Start here: **[Quest Pro room AR](Docs/Room-AR.md)** · **[Voice and model controls](Docs/Voice-And-Codex-Controls.md)** · **[White-room demo](Docs/White-Room.md)** · **[Progress and continuation](Docs/Progress-Log.md)**.

The new room-AR path uses passthrough and manually configured MRUK room data, first showing labeled outlines for wearer verification. Physical placement uses anchor-local geometry, measured prefab bounds and MRUK contact checks through the same executor and persistence system. Significant room changes retain read-only poses for save/clear/recovery; missing anchors reject restoration explicitly. See the room-AR guide for current build and hardware evidence. White-room headset results do not establish physical alignment. Downloadable catalogs and education/mentor systems remain deferred.
...
Earlier white-room validation includes real Codex composition and actual Quest microphone editing, with the original scene restored. In the AR app, the wearer confirmed aligned outlines, voice placement on the real table, and voice movement of the same orb. PC save/clear/restore preserved its exact identity, anchor and transform; the wearer confirmed the restored orb returned to the same spot on the real table. This covers one app session; restart/relocalization remains untested. The room extension passes **266 Python tests** and **236 Unity core checks per Quest build**; real Codex also passed three requests using synthetic table geometry. See the [checkpoint history](Docs/Current-Checkpoint.md), [room hardware evidence](Validation/room-ar-validation.json), and [AI setup](Docs/AI-Integration.md).
...
Unity Hub copies are available under **Desktop → Game Design → Matrix White Room Quest** and **Matrix White Room Desktop**. Use Unity **6000.6.0f1** and the exact scene paths in the [Desktop project guide](Docs/Desktop-Unity-Hub.md). The room extension synchronized 14 authored script copies across those two exports after baseline verification, preserving their scenes, prefabs and settings. The sibling **Matrix Loading Operator** checkout contains the full repository, copied builds and local saves. PRs #3–#5 were merged; room AR is on `codex/quest-pro-room-ar` in [PR #6](https://github.com/School-of-the-Ancients/matrix-loading-operator/pull/6).
