...
The catalog contains chairs, tables, walls, pedestals, blocks, orbs and columns. Furniture starts at life size. The actual Quest Pro has passed **21 direct command/save/load checks** and **61 live Codex AI loop checks**. The wearer confirms seeing the AI changes and their scene restored. The next milestone uses manually configured MRUK room targets with the same editing and persistence system. Education/mentor systems, voice and downloaded catalogs are deferred. No VaM integration is required.
...
Five real Codex CLI turns completed reviewed **spawn → double the same object's scale → save → clear → restore**, with actual Quest acknowledgements: **61 checks passed, 0 failed**. The original table and selection were restored afterward. The earlier 27/1 attempt remains recorded as history. The existing PC suite passes **136 tests**; no code changes or test reruns were needed for this successful live run. MRUK hardware, detailed tracking, floor-height and comfort checks remain separate. See the [current Quest Pro session](Validation/Quest-Pro-Session.md) and [AI setup](Docs/AI-Integration.md).
