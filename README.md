...
Earlier white-room validation includes real Codex composition and actual Quest microphone editing, with the original scene restored. The room extension passes **266 Python tests** and **236 Unity core checks**; real Codex also passed three requests using synthetic table geometry. These checks are separate from the physical-room acceptance test. See the [checkpoint history](Docs/Current-Checkpoint.md), [room provider evidence](Validation/room-context-live-codex.json), and [AI setup](Docs/AI-Integration.md).
