...
**Headset status:** the updated AR app is installed and connected; the wearer verified aligned outlines and the original orb was restored exactly. The new voice-behavior attempt captured no audible speech, and device diagnostics reported system microphone mute enabled despite granted permission. Unmuting and wearer verification of motion/persistence remain pending. This does not change the earlier completed AR voice-place → voice-move → save/clear/restore acceptance, which covered the same running app and unchanged room setup. Restart/relocalization remains untested. See [behavior evidence](Validation/behavior-validation.json), [earlier room hardware evidence](Validation/room-ar-validation.json), and [AI setup](Docs/AI-Integration.md).
...
Unity Hub copies are available under **Desktop → Game Design → Matrix White Room Quest** and **Matrix White Room Desktop**. Use Unity **6000.6.0f1** and the exact scene paths in the [Desktop project guide](Docs/Desktop-Unity-Hub.md). The sibling **Matrix Loading Operator** checkout contains the full repository, copied builds and local saves. Check the current checkpoint before assuming these copies contain the latest source. The behavior work is on `codex/runtime-prefab-behaviors`, starting from the completed room-AR checkpoint `6962eaf`; the preceding AR work is in [PR #6](https://github.com/School-of-the-Ancients/matrix-loading-operator/pull/6).
