...
The separate Meta/MRUK project's Android build remains blocked by the documented generated Meta assembly quarantine. Its real-room headset path is unverified. The subsequent independent OpenXR white-room project has built and run on Quest Pro, including a five-turn live Codex subscription loop with wearer confirmation. The composition extension has also passed 167 PC tests, 169 Unity checks per target, and 90 checks using real Codex with an actual isolated desktop player; consult the progress log for the separate current headset-composition result. Credentials stay on the PC.

After cloning, follow the README to import/build the white-room target. Raw local build logs are not part of the repository. Native MRUK work remains a later stage after its environment blocker is resolved. Keep subsequent source changes in this Git repository; the original unversioned sandbox folder was the import source, not a synchronized checkout.
