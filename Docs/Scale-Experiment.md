...
## Matrix Web desktop and Agent Portal adapters

The `/web/` desktop **Block Scale Lab** uses this same v1 request ledger. Spawn a built-in block in the virtual room and leave the Web runtime connected. On the PC, open `/clients`, create a one-use code for the browser lab, and enter that code in `/web/`. The limited client token stays in that browser tab's memory. Select the block from the lab's list and enter independent X/Y/Z factors, or select it in the Three.js view, click the canvas, and press **1–4** to propose a uniform factor. **R** proposes reset after a confirmed configure result. Both controls produce the same typed `block-scale` intent; neither changes the world until the owner reviews the exact `set_transform` command and presses **Apply** on `/clients`. The `/web/` general planner's safe-command auto-apply option is not part of this request path.

After Apply, the lab waits for the request's exact runtime receipt and shows the normalized `experiment.block-scale.observed` event only when the observation is confirmed. It reports dimensions from the built-in block's catalog bounds and final transform, plus the independent mathematical volume ratio. Its browser-saved event is labeled **historical** and shown after reload only while the saved world's room, object, anchor, asset, and numeric transform still match. Re-pair to issue new requests after reload; the service's baseline chain is not stored in the browser save. Explicitly select **Capture the current block as a new baseline** when starting a new chain from a restored transform.

The PC-local Agent Portal exposes `matrix_scale_block`, `matrix_reset_block_scale`, and `matrix_scale_status`. These tools also propose through the same v1 ledger, using a process-local limited pairing. The native Codex tool call may require its own approval; a successful tool call still only creates a proposal. The owner reviews and Applies on `/clients`, and Codex reads the confirmed result with `matrix_scale_status`. An agent request ID can be used as a baseline only within that agent pairing. These adapters do not grant the browser or agent owner Apply authority.
...
The Web adapter adds `WebRuntime/test/block_scale.test.js` for equivalent HTML/Three.js intents, a limited proposal path without Apply, confirmed-event gating, and saved-world identity checks. `ControlService/test_block_scale_math.py` checks the extracted arithmetic independently; `test_scale_agent.py` exercises the private agent bridge through owner Apply and an exact runtime receipt. These tests use synthetic scenes and do not establish headset behavior.
