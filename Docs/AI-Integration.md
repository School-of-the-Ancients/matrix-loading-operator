...
The current composition run passed **93 checks on Quest Pro** and **90 checks in an isolated Windows player**, using real Codex for both example requests. Both compositions survived PC save/clear/restore. See [Quest evidence](../Validation/composition-headset-results.json) and [desktop evidence](../Validation/composition-desktop-results.json). The original user scene was restored after testing. The wearer subsequently confirmed “it works”; [the checkpoint](Current-Checkpoint.md) preserves their newer 40-object scene. `Validation/Run-Composition-Desktop-Loop.py` provides a reusable isolated desktop runner; it requires `--run` for model calls and edits.
...
The proposal contains `commands`, `summary`, `provider`, `mode`, and `requiresApply`. When installation is needed, it also contains `contentRequests`, bounded to four exact runtime-loadable candidates from the supplied local catalog. The PC installs these first, waits for Quest registration, and replans from the updated snapshot; the final scene edit still needs reviewed Apply. `PlannerError` exposes a sanitized message and HTTP-style `status`. `ProviderConfig(base_url, model, api_key, provider)` can inject a different compatible provider without changing the Unity command executor. `validate_commands(commands, snapshot, saved_scenes=None, selection=None)` is available for independent revalidation.
