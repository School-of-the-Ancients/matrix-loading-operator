...
All 68 PC service and adapter tests passed during the white-room implementation. Coverage includes selection and placement, duplicate capacity, stale scene/selection proposals, saved-name load precedence, catalog scale, strict operation shapes, bounds, save/clear/restore, and the actual provider HTTP transport against a local mock. It also checks that keys are excluded from prompts/results/status, HTTP errors do not expose response bodies, redirects are blocked, and provider refusal/truncation/malformed JSON fails explicitly.
