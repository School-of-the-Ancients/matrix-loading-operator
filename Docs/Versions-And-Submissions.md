...
Each submission needs its own demonstration and explanation of the new contribution. The current candidate implements an explicit Quest 3/3S physical-camera-plus-virtual capture path, but actual Quest 3 permission, alignment, image-quality, and performance acceptance remains pending. Quest Pro captures remain virtual-only; changing headsets or installing a historical release does not add the new camera implementation. See [visual feedback](Visual-Feedback.md) and the [current checkpoint](Current-Checkpoint.md) for its boundaries, and [learning sessions](Learning-Sessions.md) for the earlier lesson prototype's validation scope.
