...
Movement is a direct path across an open floor, without obstacle navigation. Figures are simple orb-based markers. The demo has no social dialogue, GOAP, LLM decision loop, Agent Portal integration, School records, or headset acceptance. Its browser-local save is not the main Matrix world checkpoint or a shared multi-client simulation. [M4 Quest checks](../WebRuntime/QUEST3_ACCEPTANCE.md) remain open independently.
