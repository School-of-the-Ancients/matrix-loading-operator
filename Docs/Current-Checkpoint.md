...
## Citizens need-driven social choice candidate (September 26, 2026)

Open [PR #114](https://github.com/School-of-the-Ancients/matrix-loading-operator/pull/114) advances [#29](https://github.com/School-of-the-Ancients/matrix-loading-operator/issues/29) and builds on open [PR #113](https://github.com/School-of-the-Ancients/matrix-loading-operator/pull/113) and gives both residents a bounded social need and a conversation preference in the existing two-resident `/web/` Matrix world. At an eligible invitation opportunity, a free resident scores conversation from social deficit, preference and route distance against its legal rest/eat/explore choice. A reachable critical-hunger food choice takes precedence for either participant. A resident waiting briefly for a busy peer logs that reason once per bounded window. The invited resident's seeded response also reflects its social need. The inspector shows social need, the offered or active session, and the selected social candidate score. An offer, acceptance, decline or timeout grants no benefit; an ended session needs the matching `MatrixWorld.interact` converse receipt before both residents gain 35 social need and 12 fun and their relationship gains 5. Nested Citizens v9 persists the new fields and choice trace, migrates valid v1–v8 states, and keeps the outer world envelope at v3.

In the [isolated built `/web/` browser replay](../Validation/citizens-social-need-browser.json) on port **19853**, a prior v6 two-GLB minute-1 world migrated to v9 with its four Matrix objects. At minute **78**, Ada's social need was **42.3**; her conversation candidate scored **64.37** against the next individual activity's **19.47**, and she offered session `social-29-7`. At minute **91**, the accepted session ended with exact Matrix receipt `citizens-29-social-7-100`; both residents' social needs were **76** after the benefit, and the relationship rose from **50 to 55**. Final-build named PC world checkpoints `social-need-final-m78-offered` and `social-need-final-m91-ended` captured those states. Browser reload preserved the latter; restoring minute 78 and taking 13 **Step one minute** actions produced byte-equivalent saved world JSON at minute 91. Browser console errors: **0**. [Offered](../Validation/citizens-social-need-offered.png), [ended](../Validation/citizens-social-need-ended.png), and [receipt](../Validation/citizens-social-need-receipt.png) screenshots accompany the record. WebRuntime **314/314**, ControlService **689/689**, and the Vite build passed.
