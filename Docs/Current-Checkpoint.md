...
Current development: **[Quest Pro room AR](Room-AR.md)** on `codex/quest-pro-room-ar`, based on merged PR #5. Native MRUK source now includes labeled outlines, verified room context, surface placement, and read-only recovery. The current build/hardware status is recorded in that guide and the progress log; the historical white-room checkpoint below remains available.
