...
The updated AR APK is installed and connected; the wearer verified the outlines still align, and the original orb was restored exactly and reselected. **Behavior headset acceptance remains pending:** the latest voice attempt contained no audible speech; device diagnostics reported system microphone mute `true` while app permission was granted. The wearer reports silence persists after explicitly toggling microphone off/on. A read-only observer then captured system mute enabled throughout an actual ~5-second recording (8 samples), while the app recording client was not silenced. The responsible system/HAL policy is unknown. After restart, the wearer sees the room-loading voice line and ADB reports unauthorized; accept the fresh USB prompt and re-establish port 8765 before inspecting current state. The voice line can persist after successful room load because the loading cancellation status is not reset in `BindRoom`; check actual room state/outlines rather than assuming that line means loading is still active. No post-restart voice success is claimed. Do not count this attempt as applied AI animation or successful behavior save/restore.

The latest one-orb scene before app reopening is saved as **`BeforeVoiceRestart_20260920_175100`**. The later two-object orb/block scene is also preserved as **`BeforeBehaviorVoice_20260920_174538`**. Recheck live state before restoring; subsequent wearer edits are separate.
