...
The White Room folders are generated project copies. Their source files and metadata were checked against the built fixtures, but neither copy has been opened or rebuilt at its Desktop path. Unity will recreate its caches on first import. Lasting edits belong in the full repository; its `Build-WhiteRoom.ps1 -Target Quest` or `-Target Desktop` regenerates the corresponding `.white-room-fixture` project. Changes made only in the separate exported projects do not automatically flow back to Git.
