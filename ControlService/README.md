...
Paired clients can request AI scene proposals with `intent: {text, mode: "codex-cli"}` when discovery advertises that mode. Matrix uses the owner's existing Codex configuration and preferences; clients cannot supply models, credentials, executable paths or images. The request returns `planning` immediately, then exposes its reviewed proposal through normal polling. Only one client AI inference runs at a time, and every executable result still requires owner Apply. Offline rules and typed scale experiments remain available independently; see the [AI client lifecycle and limits](../Docs/Client-API-v1.md#optional-ai-text-planning).
...
`POST /api/plan` never changes the scene. It returns `{commands,planId,requiresApply,mode,provider,summary}`; Codex responses also contain a sanitized `inference` receipt with observed completion and token usage. The panel displays this and sends only `{planId}` to `/api/apply_plan` when Apply is clicked. Proposals expire after 120 seconds and cannot cross sessions, replay, or apply after authored scene/selection changes. Planning/applying is blocked while commands are pending. In the desktop Web virtual room, declared Citizens residents may move in X/Z without advancing the authored scene revision. A finite offline proposal that edits an unrelated object can stay reviewable; a changed selected or targeted actor stales it. Queued commands that target a resident carry a reviewed transform precondition checked by the browser before execution; a reviewed batch skips successors when a predecessor fails. AI, image, voice, and other implicit whole-scene proposals retain full actor-pose checks through Apply; pause Citizens because implicit dependencies can change before browser execution. Create a fresh proposal after a rejection.
