...
| `POST /api/plan` | `{text,mode?}` creates a state-bound proposal; mode is `offline-rules` or `openai-compatible`. |
...
To use a real OpenAI-compatible provider, set `SANDBOX_AI_BASE_URL`, `SANDBOX_AI_MODEL`, and `SANDBOX_AI_KEY` in the PC service environment. Remote providers require HTTPS and a key; localhost providers can omit a key. Existing explicit OpenAI/OpenRouter key+model settings are also recognized. A key alone does not select a model. Restart the service after changing its environment. No usable configuration was found in this session, and no live model request was made.
